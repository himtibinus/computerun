  ...
  @if($event)
      ...
      <div class="col-md-6">
        <div class="card my-3">
          <div class="card-img-top p-3 text-center button-gradient-2">
            <p class="h4 m-0 fw-bold">@if(isset($price_packages)) Pay and @endif Register</p>
          </div>
      ...
      <script>
        ...
        function provideFields(number, reserve){
          ...
          for (i = 0; i < number - 1; i++){
            ...
            draft += '<div class="form-group mb-3"><label for="team_member_' + (i + 1) + '">Team Member ' + (i + 1) + '\'s registered Email Address<b class="red-text">*</b></label><input type="email" class="form-control" name="team_member_' + (i + 1) + '" id="team_member_' + (reserve == true ? 'reserve_' : '' ) + (i + 1) + '" required onChange=\'validateUser("team_member_' + (i + 1) + '")\'><span role="alert" id="team_member_' + (i + 1) + '_validation" style="display: none"></span></div>'
          }
    ...
    @else
      <div class="col-md-6">
        <div class="card">
          <div class="card-body">
            <p class="mb-0">You have registered into this event.</p>
          </div>
        </div>
      </div>
    @endif
