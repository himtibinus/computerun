...
<?php
  $admin = false;
  if (Auth::user()->university_id == 2) $admin = true;
?>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body text-dark">
                <div class="form-group mb-4">
                    ...
                    <input name="action-update-kicker" id="action-update-kicker" type="text" value="{{ $event->kicker }}" class="form-control" @if(!$admin) disabled @endif>
                </div>
                <div class="form-group mb-4">
                    <label for="action-update-name">{{ __('Event Name') }}<b class="text-danger">*</b></label>
                    <input name="action-update-name" id="action-update-name" type="text" value="{{ $event->name }}" class="form-control" @if(!$admin) disabled @else required @endif>
                </div>
                <div class="form-group mb-4">
                    <label for="action-update-date">{{ __('Event Date') }}<b class="text-danger">*</b></label>
                    <input name="action-update-date" id="action-update-location" type="datetime-date" pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\+[0-9]{2}:[0-9]{2}" value="{{ date("c", strtotime($event->date)) }}" class="form-control" @if(!$admin) disabled @else required @endif>
                </div>
                <div class="form-group mb-4">
                    <label for="action-update-location">{{ __('Event Location') }}</label>
                    <input name="action-update-location" id="action-update-location" type="text" value="{{ $event->location }}" class="form-control" @if(!$admin) disabled @endif>
                </div>
                <div class="form-group mb-4">
                    <label for="action-update-price">{{ __('Event Price (Rp)') }}<b class="text-danger">*</b></label>
                    <input name="action-update-price" id="action-update-price" type="number" min="0" value="{{ $event->price }}" class="form-control" @if(!$admin) disabled @else required @endif>
                </div>
                <div class="form-group mb-4">
                    <label for="action-update-cover_image">{{ __('Cover Image (URL)') }}</label>
                    <input name="action-update-cover_image" id="action-update-cover_image" type="text" value="{{ $event->cover_image }}" class="form-control" @if(!$admin) disabled @endif>
                </div>
            </div>
        </div>
        <div class="card mb-4">
            ...
            <div class="card-body text-dark">
                ...
                <div class="form-group mb-4">
                    <textarea name="action-update-description_public" id="action-update-description_public" type="text" rows="4" class="form-control" @if(!$admin) disabled @endif>{{ $event->description_public }}</textarea>
                </div>
            </div>
        </div>
        <div class="card mb-4">
            ...
            <div class="card-body text-dark">
                ...
                <div class="form-group mb-4">
                    <textarea name="action-update-description_pending" id="action-update-description_pending" type="text" rows="4" class="form-control" @if(!$admin) disabled @endif>{{ $event->description_pending }}</textarea>
                </div>
            </div>
        </div>
        <div class="card mb-4">
            ...
            <div class="card-body text-dark">
                ...
                <div class="form-group mb-4">
                    <textarea name="action-update-description_private" id="action-update-description_private" type="text" rows="4" class="form-control" @if(!$admin) disabled @endif>{{ $event->description_private }}</textarea>
                </div>
            </div>
        </div>
        <div class="card mb-4">
            ...
            <div class="card-body text-dark">
                <div class="form-group mb-4">
                    ...
                    <select name="action-registration-status" id="action-registration-status" @if(!$admin) disabled @endif>
                ...
                <div class="form-group mb-4">
                    ...
                    <select name="action-registration-private" id="action-registration-private" @if(!$admin) disabled @endif>
                        <option value="-1">Unchanged</option>
                ...
                <div class="form-group mb-4">
                    ...
                    <select name="action-registration-auto_accept" id="action-registration-auto_accept" @if(!$admin) disabled @endif>
                        <option value="-1">Unchanged</option>
                        <option value="enabled">Yes</option>
                        <option value="disabled">No</option>
                    </select>
                ...
                <div class="form-group mb-4">
                    ...
                    <input name="action-update-seats" id="action-update-seats" type="number" value="{{ $event->seats }}" min="1" class="form-control" @if(!$admin) disabled @else required @endif>
                </div>
                <div class="form-group mb-4">
                    <label for="action-update-slots">{{ __('Maximum Tickets per User') }}</label>
                    <input name="action-update-slots" id="action-update-slots" type="number" value="{{ $event->slots }}" min="1" class="form-control" @if(!$admin) disabled @else required @endif>
                </div>
                <div class="form-group mb-4">
                    <label for="action-update-team_members">{{ __('Required Team Members') }}</label>
                    <input name="action-update-team_members" id="action-update-team_members" type="number" value="{{ $event->team_members }}" min="0" class="form-control" @if(!$admin) disabled @else required @endif>
                </div>
                <div class="form-group mb-4">
                    <label for="action-update-team_members_reserve">{{ __('Reserve Team Members') }}</label>
                    <input name="action-update-team_members_reserve" id="action-update-team_members_reserve" type="number" value="{{ $event->team_members_reserve }}" min="0" class="form-control" @if(!$admin) disabled @else required @endif>
                </div>
            </div>
        </div>
        <div class="card mb-4">
            ...
            <div class="card-body text-dark">
                <div class="form-group mb-4">
                    ...
                    <select name="action-attendance-status" id="action-attendance-status" @if(!$admin) disabled @endif>
                        <option value="-1">Unchanged</option>
                        <option value="enabled">Yes</option>
                        <option value="disabled">No</option>
                    </select>
                ...
                <div class="form-group mb-4">
                    ...
                    <select name="action-attendance-type" id="action-attendance-type" @if(!$admin) disabled @endif>
                        <option value="-1">Unchanged</option>
                ...
                <div class="form-group mb-4">
                    ...
                    <input name="action-update-url_link" id="action-update-url_link" type="text" value="{{ $event->url_link }}" class="form-control" @if(!$admin) disabled @endif>
                </div>
                <div class="form-group mb-4">
                    ...
                    <input name="action-update-totp_key" id="action-update-totp_key" type="text" value="{{ $event->totp_key }}" class="form-control" @if(!$admin) disabled @endif>
                </div>
            </div>
        </div>
        @if($admin == true)
                ...
                <div class="btn-toolbar" role="toolbar">
                    ...
                    @if($registration->file_id != null && $registration->file_id != '')
                        ...
                        @if (is_object($file) && $file->name != null)
                            <div class="btn-group mr-2" role="group">
                                <a href="/admin/downloadFile/1/{{$registration->file_id}}" class="btn btn-secondary" onClick="requestParticipantDetails('{{ $registration->email }}')">
                                    <i class="bi bi-file-earmark-richtext"></i> Download Supporting File
                                </a>
                            </div>
                        @endif
